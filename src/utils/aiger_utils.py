...
from utils.cnf_utils import kissat_solve, read_cnf
from utils.dag_utils import return_forward_order_info

# def solve_aig(aig_filename, tmp_dir, args=None):
#     design_name = aig_filename.split('/')[-1].split('.')[0]
#     tmp_cnf_filename = os.path.join(tmp_dir, 'tmp_{}.cnf'.format(design_name))
#     cmd = 'aigtocnf {} {}'.format(aig_filename, tmp_cnf_filename)
#     info = os.popen(cmd).readlines()
#     cnf, no_vars = read_cnf(tmp_cnf_filename)
    
#     # Solve 
#     sat_res, asg, solvetime = kissat_solve(cnf, no_vars, args=args)
    
#     return sat_res, asg, solvetime, no_vars, len(cnf)

def solve_aig(aig_filename, tmp_dir, args=None):
    # Parse AIG
    design_name = aig_filename.split('/')[-1].split('.')[0]
    tmp_aag_filename = os.path.join(tmp_dir, 'tmp_{}.aag'.format(design_name))
    x_data, edge_index = aig_to_xdata(aig_filename, tmp_aag_filename)
    ...
    return sat_res, asg, solvetime, len(x_data), len(cnf)
...
#     # Solve
#     fanin_list, fanout_list = circuit_utils.get_fanin_fanout(x_data, edge_index)
#     x_data, level_list = circuit_utils.feature_gen_level(x_data, fanout_list)
#     assert len(level_list[-1]) == 1
#     po_idx = level_list[-1][0]
#     cnf = xdata_to_cnf(x_data, fanin_list, gate_to_index={'PI': 0, 'AND': 1, 'NOT': 2}, const_0=[], const_1=[po_idx])
#     sat_res, asg, solvetime = kissat_solve(cnf, len(x_data), args=args)
...
def xdata_to_cnf(data, fanin_list, gate_to_index, const_0=[], const_1=[], add_clauses=[]):
    cnf = []
